...
import { Send, Terminal, Star } from 'lucide-react';
...
const Contact = () => {
  ...
  return (
    <section id="feedback" className="py-24 bg-[#1a1f22] relative">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-2xl md:text-4xl font-stencil text-gunmetal dark:text-white tracking-widest uppercase">PANEL EVALUASI OPERASIONAL</h2>
          </div>
          <p className="text-gray-400 mb-8 font-mono text-sm">BERIKAN PENILAIAN ATAU PENGADUAN TERKAIT SISTEM HELPDESK DART.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-gray-600 dark:text-gray-400 font-tactical text-sm tracking-wider mb-2">KATEGORI EVALUASI</label>
                <select 
                  value={data.kategori}
                  onChange={e => setData('kategori', e.target.value)}
                  className="w-full bg-sand dark:bg-gunmetal border border-gray-600 p-3 text-gunmetal dark:text-white focus:outline-none focus:border-olive focus:ring-1 focus:ring-olive transition-all cursor-pointer"
                >
                  <option>Saran Sistem</option>
            ...
            <div>
              <label className="block text-gray-600 dark:text-gray-400 font-tactical text-sm tracking-wider mb-2">TINGKAT KEPUASAN (RATING)</label>
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    ...
                    className={`p-2 transition-all ${data.rating >= star ? 'text-yellow-500 scale-110' : 'text-gray-600 dark:text-gray-700 hover:text-yellow-500/50'}`}
                  >
            ...
            <div>
              <label className="block text-gray-600 dark:text-gray-400 font-tactical text-sm tracking-wider mb-2">RINCIAN EVALUASI (ENCRYPTED)</label>
              <textarea 
                required
                rows={4}
                value={data.pesan}
            ...
            >
              {!processing && !wasSuccessful && <><Send className="w-5 h-5 group-hover:translate-x-1 transition-transform" /> KIRIM TRANSMISI</>}
              {processing && <span className="typewriter-text">ENKRIPSI DATA...</span>}
              {wasSuccessful && <span className="text-khaki">TRANSMISI BERHASIL</span>}
            </button>
          </form>
        </motion.div>
      </div>
