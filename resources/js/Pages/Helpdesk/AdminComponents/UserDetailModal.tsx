...
const UserDetailModal: React.FC<UserDetailModalProps> = ({ isOpen, onClose, user }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-slate-50 dark:bg-gunmetal border-2 border-olive w-full max-w-lg shadow-[0_0_50px_rgba(75,83,32,0.4)]">
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 gap-y-4 gap-x-8">
            <div className="col-span-2 flex items-center gap-4 mb-4 pb-4 border-b border-slate-200 dark:border-slate-700/50">
            ...
            <div className="col-span-2">
              ...
              <p className={`text-sm font-mono font-bold p-2 bg-gray-200 dark:bg-navy/80 border border-slate-200 dark:border-slate-700/50 ${user.role !== 'Teknisi' ? 'text-slate-500 italic' : 'text-gunmetal dark:text-blue-400'}`}>
                {user.role === 'Teknisi' ? (user.spesialisasi || 'BELUM DIATUR') : 'TIDAK TERSEDIA (NON-TEKNISI)'}
              </p>
            </div>
          </div>

          <div className="mt-8 pt-4 border-t border-slate-200 dark:border-slate-700/50 flex justify-end">
            <button
              onClick={onClose}
              className="bg-gunmetal dark:bg-navy text-white px-8 py-2 font-tactical font-bold tracking-widest hover:bg-slate-700 transition-colors border border-slate-500"
            >
              TUTUP
            </button>
          </div>
