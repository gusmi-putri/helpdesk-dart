...
const PelaporTopbar: React.FC<PelaporTopbarProps> = ({ setIsMobileMenuOpen, currentUser }) => {
  return (
    <header className="h-16 border-b border-slate-200 dark:border-slate-600 bg-cighra-primary text-white dark:bg-cighra-darkcard/60 backdrop-blur-md flex items-center justify-between px-4 md:px-8 flex-shrink-0 z-10 relative">
      ...
      <div className="flex items-center gap-0 border border-slate-200/20 dark:border-slate-600 rounded shadow-sm bg-cighra-light dark:bg-cighra-darkcard/80 ml-auto">
        <div className="bg-sand/80 dark:bg-cighra-darkcard/80 px-4 py-1.5 text-right flex flex-col justify-center">
          ...
          <span className="block text-[9px] font-mono tracking-widest text-cighra-gold dark:text-cighra-gold">{currentUser?.id || 'PELAPOR'}</span>
        </div>
        <div className="w-10 h-full bg-cighra-light dark:bg-cighra-darkcard/70 border-l border-slate-200 dark:border-slate-600 flex items-center justify-center p-2">
