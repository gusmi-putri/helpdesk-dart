...
const PelaporSidebar: React.FC<PelaporSidebarProps> = ({
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-300 w-72 bg-sand dark:bg-black border-r border-soft-gunmetal/10 dark:border-soft-sand/5 z-50 flex-shrink-0 flex flex-col shadow-2xl`}>
      <div className="p-6 border-b border-soft-gunmetal/10 dark:border-soft-sand/5 flex items-center gap-4 bg-sand/50 dark:bg-[#050505]">
      ...
      <nav className="flex-1 overflow-y-auto custom-scrollbar py-6">
        <button
          ...
          className={`w-full flex items-center gap-3 px-6 py-4 font-tactical text-sm tracking-wider transition-all border-l-4
            ${activeMenu === 'FORM' ? 'bg-sand dark:bg-gunmetal/40 text-targetred border-targetred shadow-inner' : 'border-transparent text-soft-gunmetal/60 dark:text-soft-sand/40 hover:bg-sand/50 dark:hover:bg-gunmetal/20'}
          `}
        >
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-6 py-4 font-tactical text-sm tracking-wider transition-all border-l-4
            ${activeMenu === 'HISTORY' ? 'bg-sand dark:bg-gunmetal/40 text-targetred border-targetred shadow-inner' : 'border-transparent text-soft-gunmetal/60 dark:text-soft-sand/40 hover:bg-sand/50 dark:hover:bg-gunmetal/20'}
          `}
        >
