...
const ReportForm: React.FC<ReportFormProps> = ({
  ...
  return (
    <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
      <div className="glass-panel border-t-4 border-t-olive overflow-hidden bg-white dark:bg-navy/80 shadow-xl border border-slate-200 dark:border-slate-700">
      ...
      <form onSubmit={handleFormSubmit} className="space-y-6">
        <div className="glass-panel p-6 border-l-4 border-l-olive space-y-5 bg-white dark:bg-navy/80 shadow-xl border border-slate-200 dark:border-slate-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input type="text" readOnly value={authUser?.nama_lengkap || currentUser?.name || ''} className="w-full bg-white dark:bg-navy/80 border border-slate-300 dark:border-slate-600 px-4 py-2.5 text-sm text-gunmetal dark:text-slate-300 cursor-not-allowed rounded-sm" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-300 mb-1.5">Pangkat / NRP / Golongan</label>
              <input type="text" readOnly value={authUser?.nrp_nip || '-'} className="w-full bg-white dark:bg-navy/80 border border-slate-300 dark:border-slate-600 px-4 py-2.5 text-sm text-gunmetal dark:text-slate-300 cursor-not-allowed rounded-sm" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-300 mb-1.5">Satuan Kerja</label>
              <input type="text" readOnly value={authUser?.asal_satuan || '-'} className="w-full bg-white dark:bg-navy/80 border border-slate-300 dark:border-slate-600 px-4 py-2.5 text-sm text-gunmetal dark:text-slate-300 cursor-not-allowed rounded-sm" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-300 mb-1.5">Nomor WhatsApp Aktif</label>
              <input type="text" readOnly value={authUser?.no_wa || '-'} className="w-full bg-white dark:bg-navy/80 border border-slate-300 dark:border-slate-600 px-4 py-2.5 text-sm text-gunmetal dark:text-slate-300 cursor-not-allowed rounded-sm" />
            </div>
          </div>
        ...
        <div className="glass-panel p-6 border-l-4 border-l-olive !overflow-visible relative z-20 bg-white dark:bg-navy/80 shadow-xl border border-slate-200 dark:border-slate-700">
          <SearchableSelect 
        ...
        <div className="glass-panel p-6 border-l-4 border-l-olive bg-white dark:bg-navy/80 shadow-xl border border-slate-200 dark:border-slate-700">
          <label className="block text-sm font-semibold text-slate-600 dark:text-slate-300 mb-3 uppercase">Tingkat Kerusakan <span className="text-targetred">*</span></label>
          <div className="space-y-3">
            {['Ringan', 'Sedang', 'Parah'].map(level => (
              <label key={level} className={`flex items-center gap-3 p-3 rounded-sm border cursor-pointer transition-all ${data.tingkat_kerusakan === level ? 'border-olive bg-olive/10 dark:bg-olive/20' : 'border-slate-200 dark:border-slate-700 hover:border-olive/50 hover:bg-white dark:hover:bg-black/40'}`}>
        ...
        <div className="glass-panel p-6 border-l-4 border-l-olive bg-white dark:bg-navy/80 shadow-xl border border-slate-200 dark:border-slate-700">
          <label className="block text-sm font-semibold text-slate-600 dark:text-slate-300 mb-1 uppercase">Upload Bukti Kendala <span className="text-targetred">*</span></label>
          ...
          {data.file_bukti.length > 0 && (
            <div className="mt-4 space-y-2">
              {data.file_bukti.map((file: any, i: number) => (
                <div key={i} className="flex items-center justify-between p-2.5 bg-white dark:bg-navy/80 border border-slate-200 dark:border-slate-700 rounded-sm">
                  ...
                  <button type="button" onClick={() => removeFile(i)} className="text-slate-500 hover:text-targetred transition-colors p-1"><Trash2 size={14} /></button>
                </div>
              ))}
        ...
        <div className="glass-panel p-6 border-l-4 border-l-olive bg-white dark:bg-navy/80 shadow-xl border border-slate-200 dark:border-slate-700">
          <label className="block text-sm font-semibold text-slate-600 dark:text-slate-300 mb-1 uppercase">Prioritas Penanganan <span className="text-targetred">*</span></label>
          ...
          <div className="space-y-3">
            {[
              ...
              <label key={opt.val} className={`flex items-center gap-3 p-3 rounded-sm border cursor-pointer transition-all ${data.urgensi === opt.val ? 'border-olive bg-olive/10 dark:bg-olive/20' : 'border-slate-200 dark:border-slate-700 hover:border-olive/50 hover:bg-white dark:hover:bg-black/40'}`}>
        ...
        <div className="glass-panel p-6 border-l-4 border-l-olive bg-white dark:bg-navy/80 shadow-xl border border-slate-200 dark:border-slate-700">
          <label className="block text-sm font-semibold text-slate-600 dark:text-slate-300 mb-2 uppercase">Deskripsi Kerusakan <span className="text-targetred">*</span></label>
          <textarea value={data.deskripsi} onChange={(e) => setData('deskripsi', e.target.value)} required rows={5}
            className="w-full bg-slate-50/50 dark:bg-navy/80 border border-slate-300 dark:border-slate-600 px-4 py-3 text-sm text-slate-800 dark:text-white focus:outline-none focus:border-olive transition-colors resize-none rounded-sm"
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setIsConfirmOpen(false)}>
          <div className="bg-white dark:bg-gunmetal border border-olive/30 shadow-2xl max-w-lg w-full mx-4 rounded-sm overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 bg-olive/10 border-b border-olive/20 flex items-center justify-between">
              ...
              <button onClick={() => setIsConfirmOpen(false)} className="text-slate-500 dark:text-slate-300 hover:text-targetred transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="bg-white dark:bg-navy/80 p-3 border border-olive/20 text-xs font-mono space-y-1">
