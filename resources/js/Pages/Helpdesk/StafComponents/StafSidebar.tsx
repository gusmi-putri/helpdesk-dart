...
import { AlertTriangle, CheckCircle, Package, GitPullRequest, Users, Layers, ChevronDown, ChevronRight, MapPin } from 'lucide-react';
...
const StafSidebar: React.FC<StafSidebarProps> = ({
  ...
  const menuClass = (menu: string) =>
    `w-full flex items-center justify-between px-6 py-3.5 font-tactical text-sm tracking-wider transition-all duration-300 border-l-4 group focus-visible:ring focus-visible:ring-cighra-gold focus-visible:outline-none
      ...
      >
        ...
        <nav className="flex-1 py-6 space-y-1 overflow-y-auto custom-scrollbar">
          ...
          >
            ...
            {pendingCount > 0 && (
              <span
                className="bg-cighra-primary dark:bg-cighra-gold dark:text-slate-900 text-white text-xs font-bold px-2 py-0.5 rounded-full animate-pulse"
            ...
            {isDataMasterExpanded && (
              ...
              >
                <button
                  ...
                  className={`w-full text-left pl-[54px] py-2.5 flex items-center gap-2 text-xs font-tactical tracking-widest transition-all duration-300 focus-visible:ring group focus-visible:ring-cighra-gold focus-visible:outline-none ${activeMenu === 'PERSONEL' ? 'text-cighra-primary dark:text-cighra-gold font-bold' : 'text-slate-500 dark:text-slate-300 hover:text-cighra-primary dark:hover:text-cighra-gold'}`}
                >
                ...
                <button
                  ...
                  className={`w-full text-left pl-[54px] py-2.5 flex items-center gap-2 text-xs font-tactical tracking-widest transition-all duration-300 focus-visible:ring group focus-visible:ring-cighra-gold focus-visible:outline-none ${activeMenu === 'INVENTARIS' ? 'text-cighra-primary dark:text-cighra-gold font-bold' : 'text-slate-500 dark:text-slate-300 hover:text-cighra-primary dark:hover:text-cighra-gold'}`}
                >
                ...
                <button
                  ...
                  className={`w-full text-left pl-[54px] pr-4 py-2.5 flex items-center justify-between text-xs font-tactical tracking-widest transition-all duration-300 focus-visible:ring group focus-visible:ring-cighra-gold focus-visible:outline-none ${activeMenu === 'MUTASI' ? 'text-cighra-primary dark:text-cighra-gold font-bold' : 'text-slate-500 dark:text-slate-300 hover:text-cighra-primary dark:hover:text-cighra-gold'}`}
                >
