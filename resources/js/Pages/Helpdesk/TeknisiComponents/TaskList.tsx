...
const TaskList: React.FC<TaskListProps> = ({
  ...
  return (
    <div className="xl:col-span-4 space-y-4">
      <div className="bg-white/60 dark:bg-navy/80 border-b-2 border-olive p-4 shadow-md space-y-4">
        ...
        <div className="flex gap-1 bg-slate-50 dark:bg-navy/80 p-1 border border-slate-200 dark:border-slate-700">
          <button 
            onClick={() => { setActiveTab('ACTIVE'); setSelectedTaskId(null); }}
            className={`flex-1 py-1.5 text-[10px] font-tactical font-bold transition-all uppercase ${activeTab === 'ACTIVE' ? 'bg-olive text-sand shadow-lg' : 'text-slate-500 dark:text-slate-300 hover:text-gunmetal dark:hover:text-white'}`}
          >
            DAFTAR TUGAS
          </button>
          <button 
            onClick={() => { setActiveTab('HISTORY'); setSelectedTaskId(null); }}
            className={`flex-1 py-1.5 text-[10px] font-tactical font-bold transition-all uppercase ${activeTab === 'HISTORY' ? 'bg-targetred text-sand shadow-lg' : 'text-slate-500 dark:text-slate-300 hover:text-gunmetal dark:hover:text-white'}`}
          >
            RIWAYAT SELESAI
          </button>
        </div>
        ...
        <div className="relative">
          <input 
            ...
            className="w-full bg-white dark:bg-gunmetal/30 border border-slate-200 dark:border-slate-700 p-2 pl-8 text-[10px] font-mono focus:outline-none focus:border-olive uppercase text-slate-800 dark:text-white"
          />
      ...
      <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
        {tasks.length === 0 ? (
          <div className="p-6 border border-slate-200 dark:border-slate-700 bg-white/40 dark:bg-navy/80 text-center font-mono text-slate-500 dark:text-slate-300 rounded-sm uppercase text-xs">
            ...
            <div
              ...
              className={`p-4 border-2 transition-all cursor-pointer rounded-sm group
                ${selectedTaskId === task.db_id
                  ...
                  : 'border-transparent bg-white/60 dark:bg-navy/80 hover:border-soft-gunmetal/20 dark:hover:border-slate-600 shadow-sm'
                }
              `}
            >
              ...
              <h3 className="text-slate-800 dark:text-white font-tactical text-lg font-bold mb-1 group-hover:text-olive transition-colors leading-tight line-clamp-2 uppercase">
